...
export class BLEService implements IOBDAdapter {
      ...
      this.manager.startDeviceScan(null, null, (error, device) => {
        if (error) {
          ...
          this.setState(ConnectionState.DISCONNECTED);
           this.manager.stopDeviceScan(); // Stop scan on error
          resolve([]);
          return;
        }

        // Filter for devices that look like OBD adapters
        if (device && device.name && (device.name.includes("OBD") || device.name.includes("Vlink") || device.name.includes("ELM"))) {
  ...
  async sendCommand(command: string): Promise<string> {
    if (!this.device || this.connectionState !== ConnectionState.CONNECTED && this.connectionState !== ConnectionState.READY && this.connectionState !== ConnectionState.STREAMING) {
